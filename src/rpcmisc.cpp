...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"mymnaddress\"\n"
            "\nReturn information about the given mymn address.\n"
            "\nArguments:\n"
            "1. \"mymnaddress\"     (string, required) The mymn address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mymnaddress\", (string) The mymn address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Mymn address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are mymn addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) mymn address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"mymnaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"mymnaddress\"  (string, required) The mymn address to use for the signature.\n"
